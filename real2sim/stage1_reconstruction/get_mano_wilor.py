...
def main(
    ...
    output_dir: str='./demo_data/mano/arthur_tyler_pass_by_nov20/cam01',
    ...
    for img_path in tqdm(img_paths):
        ...
        print(f"Processing image: {img_path}, frame_idx: {frame_idx}")
        pose2d_path = Path(pose2d_dir) / f'pose_{frame_idx:05d}.json'
        # if pose2d_path does not exist, continue
        if not pose2d_path.exists():
            print(f"[SKIP] pose2d JSON not found for frame {frame_idx}: {pose2d_path}")
            continue
        ...
        for person_id in person_ids:
            ...
            if left_hand_joints[:, 2].mean() > hand_bbox_thr:
                ...
                print(f"[SKIP] Low left hand confidence for person {person_id} in frame {frame_idx}")
            ...
            if right_hand_joints[:, 2].mean() > hand_bbox_thr:
                ...
                print(f"[SKIP] Low right hand confidence for person {person_id} in frame {frame_idx}")
        ...
        if len(is_rights) == 0:
            print(f"[SKIP] No valid hand bounding boxes for frame {frame_idx}")
            continue
        assert len(is_rights) == len(bboxes) == len(person_id_list)
        bboxes = np.array(bboxes, dtype=np.float32)
        
        # print the inputs
        print(f"Input bboxes: {bboxes}")
        print(f"Input is_rights: {is_rights}")
        print(f"Input person_id_list: {person_id_list}")

        outputs = pipe.predict_with_bboxes(image, bboxes=bboxes, is_rights=is_rights)

        if len(outputs) == 0:
            print(f"[SKIP] No outputs from pipeline for frame {frame_idx}")
            continue
